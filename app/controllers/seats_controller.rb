class SeatsController < ApplicationController
  ...
  before_action :find_seat_id, only: [:show, :edit, :update, :destroy]
  ...
  def create
    ...
    if @seat.save
      redirect_to restaurant_path(@restaurant), notice: "成功新增位子類型"
    else
      render :new
    end
  end
  ...
  def update
    if @seat.update(params_seat)
      redirect_to restaurant_path(@seat.restaurant), notice: "編輯seat成功"
    else
      render :edit
    end
  end
  ...
  def destroy
    @seat.destroy
    redirect_to restaurant_path(@seat.restaurant), alert: "刪除seat成功"
  end
